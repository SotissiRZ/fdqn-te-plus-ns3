...
class FDQNServer:
    ...
    def __init__(self):
        ...
        print(f"\n{'='*60}")
        print(f"FDQN-TE+ Server — Variante : DQN_noPEPM")
        print(f"  PEPM désactivé : risk=0.0 fixe, pas de LSTM")
        print(f"{'='*60}")
    ...
    def _handle_pepm_batch(self, payload: dict) -> dict:
        # ABLATION noPEPM : PEPM désactivé.
        # Retourne risk=0.0 pour tous les nœuds sans appeler le LSTM.
        # Le pool PEPM n'est pas mis à jour → aucun historique accumulé.
        nodes_payload = payload.get("nodes", [])
        risks = {str(int(e.get("node_id", 0))): 0.0 for e in nodes_payload}
        return {
            "risks":     risks,
            "at_risk":   [],
    ...
    def _handle_pepm(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        # ABLATION noPEPM : PEPM désactivé — risk=0.0 fixe, pas d'appel LSTM.

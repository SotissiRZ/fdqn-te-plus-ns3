"""
rl_server_noIFO.py — Serveur FDQN-TE+ complet, ablation IFO uniquement
=======================================================================
Identique à rl_server.py (DQN + PEPM + FedMeta-DRL) — AUCUNE modification.
Le clustering LEACH probabiliste est géré entièrement côté C++ (fdqn_noIFO.cc).
Ce serveur reçoit la topologie LEACH comme n'importe quelle autre topologie
et l'utilise pour la fédération — il n'a pas besoin de savoir comment les
clusters ont été formés.

Port : 5558  (dédié ablation IFO)
"""
...
class FDQNServer:
    ...
    def _handle_reward(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return_val = {
            "ok":        True,
            "loss":      float(loss),
            "federated": should_federate   # [FIX-S1] C++ lit ce champ via ParseBool
        }
...
def run_server(host: str = "0.0.0.0", port: int = 5558):
    """Lance le serveur"""

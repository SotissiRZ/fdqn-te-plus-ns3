...
class FDQNServer:
    ...
    def __init__(self):
        ...
        print(f"\n{'='*60}")
        print(f"FDQN-TE+ Server — Variante : DQN_noFed")
        print(f"  Fédération désactivée : apprentissage purement local")
        print(f"{'='*60}")
    ...
    def _handle_reward(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # ABLATION noFed : Fédération désactivée — should_federate toujours False.
        # Les agents apprennent localement (ADDQN individuel) sans agrégation FedAvg.

        return_val = {
            "ok":               True,
            "loss":             float(loss),
            "federated":        False,  # ABLATION noFed : jamais fédéré
            "aggregation_ok":   0,      # [FIX-F] Toujours 0 → C++ ne peut pas incrémenter m_fedRound
            "fed_round":        0,
            "agg_participants": 0,
        }

        # Pas de thread _run_federation lancé
        return return_val
    ...
    def _run_federation(self):
        """ABLATION noFed : Fédération désactivée — corps neutralisé (jamais appelé)."""
        return
...
def run_server(host: str = "0.0.0.0", port: int = 5557):
    """Lance le serveur"""

...
class FDQNServer:
    ...
    def _handle_action(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # step_advance() déplacé dans _handle_reward : le compteur fédéral
        # doit avancer au moment de la soumission de récompense, pas de l'action,
        # pour rester synchrone avec should_run() qui évalue FED_PERIOD rewards.
    ...
    def _handle_reward(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Avancer le compteur fédéral ICI (sur chaque reward reçu, pas sur action)
        # → should_run() déclenche FedAvg tous les FED_PERIOD rewards réels.
        # Synchronise le compteur C++ (qui lit "federated" dans la réponse)
        # avec le vrai rythme d'agrégation Python.
        ...
        # [FIX-S1] Inclure "federated" dans la réponse JSON.
        #
        # ANOMALIE ORIGINALE :
        #   return_val = {"ok": True, "loss": float(loss)}  — sans "federated"
        #   → C++ (fdqn_te_plus_eval.cc) lisait ParseBool("federated", false)
        #     → retournait toujours false → m_fedRound ne s'incrémentait jamais
        #     pour FDQN-TE+ non plus (après FIX-C1 côté C++).
        #
        # CORRECTION :
        #   Retourner "federated": should_federate (True si round fédéré lancé).
        #   Le C++ lit ce champ et incrémente m_fedRound uniquement quand True.
        #   → log "fed=N" reflète exactement les rondes FedAvg réelles effectuées.
        return_val = {
            "ok":        True,
            "loss":      float(loss),
            "federated": should_federate   # True → C++ incrémente m_fedRound
        }
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Serveur RL FDQN-TE+")
    parser.add_argument("--port", type=int, default=FdqnConfig.RL_PORT,
                        help=f"Port TCP d'écoute (défaut: {FdqnConfig.RL_PORT})")

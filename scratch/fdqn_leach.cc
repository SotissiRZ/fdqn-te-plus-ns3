...
struct SimParams {
    ...
    uint32_t seed          = 42;
    int rlPort = FdqnCfg::RL_PORT; // 5555 par défaut et 5559
    std::string resultsDir = "results/DQN_LEACH";
};
...
public:
    ...
    void SubmitReward(const std::vector<double>& state, int action, double reward,
        ...
        if (SendLine(os.str())) {
            std::string resp = RecvLine();
            m_steps++;
            // DQN-noIFO conserve PEPM et fédération → réponse "federated": true
            // quand la ronde fédérée a lieu → m_fedRound reflète les vraies rondes.
            if (ParseBool(resp, "federated", false)) m_fedRound++;
        }
    }
...
private:
    ...
    bool ParseBool(const std::string& s, const std::string& key, bool def) {
        const std::string k = "\"" + key + "\":";
        auto pos = s.find(k);
        if (pos == std::string::npos) return def;
        pos += k.size();
...
double ComputeRLLogicalDelay_ms(const std::vector<NodeState>& nodes,
    ...
    static constexpr double PHY_RATE_BPS     = 1.0e6;
...
static void InitDoCheck() {
    doCheck = [](int round) {
        ...
        // ── ABLATION noIFO : Re-clustering LEACH probabiliste (périodique) ─────────
        if (round % 2 == 0 && alive > 0) {
            const double k_opt_dyn = static_cast<double>(FdqnCfg::N_CLUSTERS)
                                   / static_cast<double>(alive);

            std::uniform_real_distribution<double> dist01dyn(0.0, 1.0);
            std::vector<uint32_t> chListDyn;

            for (auto& ns : *ctx.pSt) {
                if (!ns.isAlive) continue;
                ns.isClusterHead = (dist01dyn(*ctx.pRng) < k_opt_dyn);
                if (ns.isClusterHead) {
                    ns.clusterId = ns.id;
                    chListDyn.push_back(ns.id);
                }
            }
            ...
            std::map<uint32_t, std::vector<uint32_t>> chMems;
            for (const auto& cid : chListDyn) chMems[cid] = {};

            for (auto& ns : *ctx.pSt) {
                if (!ns.isAlive || ns.isClusterHead) continue;
                uint32_t bestCH   = chListDyn[0];
                double   bestDist = 1e18;
                for (uint32_t cid : chListDyn) {
                    for (const auto& cn : *ctx.pSt) {
                        if (cn.id != cid || !cn.isAlive) continue;
                        const double d = NodeDist(ns.x, ns.y, cn.x, cn.y);
                        if (d < bestDist) { bestDist = d; bestCH = cid; }
                        break;
                    }
                }
                ns.clusterId = bestCH;
                chMems[bestCH].push_back(ns.id);
            }

            std::vector<ClusterInfo> newClusters;
            for (const auto& [cid, mems] : chMems) {
                ClusterInfo ci;
                ci.chId    = cid;
                ci.members = mems;
                newClusters.push_back(ci);
            }
            g_ifo.SetClustersFromExternal(newClusters);
            ...
            NS_LOG_UNCOND("[DQN-LEACH] Recluster round=" << round
                        << " → " << chListDyn.size() << " clusters (LEACH probabiliste)");
        ...
        if (ctx.pEnergyCSV && ctx.pEnergyCSV->is_open()) {
            ...
            *ctx.pEnergyCSV << std::fixed
                << round << ","
                << std::setprecision(1) << now << ","
                << alive << "," << dead << ","
                << std::setprecision(6) << meanE << ","
                << stdE << ","
                << minE << ","
                << (alive>0 ? maxE : 0.0) << ","
                << std::setprecision(4) << drainedTotal << ","
                << std::setprecision(2) << pdrRLNow << ","
                << std::setprecision(2) << pdrRLRound << ","
                << pdrNS3Now << ","
                << std::setprecision(2) << curDelay << ","
                << atRisk << ","
                << std::setprecision(4) << pepmRiskMean << ","
                << std::setprecision(1)
                << (g_fndDone ? g_fndTime : 0.0) << ","
                << (g_hndDone ? g_hndTime : 0.0) << ","
                << (g_lndDone ? g_lndTime : 0.0) << ","
                << g_rl.GetSteps() << ","
                << g_rl.GetFedRound() << ","
                << ctx.pIfo->GetRound() << ","
                << nClusters << ","
...
int main(int argc, char* argv[]) {
    ...
    g_ifo.SetArea(p.sinkX, p.sinkY, p.areaSize, p.radioRange, p.initEnergy, p.ifoIterations);

    // ── ABLATION noIFO : Clustering LEACH probabiliste classique ────────────────
    {
        const double k_opt = static_cast<double>(FdqnCfg::N_CLUSTERS)
                           / static_cast<double>(p.nNodes);

        std::uniform_real_distribution<double> dist01(0.0, 1.0);

        std::vector<ClusterInfo> leachClusters;
        std::vector<uint32_t> chList;

        for (auto& ns : nodeStates) {
            if (dist01(rng) < k_opt) {
                ns.isClusterHead = true;
                ns.clusterId     = ns.id;
                chList.push_back(ns.id);
            } else {
                ns.isClusterHead = false;
            }
        }

        if (chList.empty()) {
            nodeStates[0].isClusterHead = true;
            nodeStates[0].clusterId     = nodeStates[0].id;
            chList.push_back(nodeStates[0].id);
        }

        std::map<uint32_t, std::vector<uint32_t>> chMembers;
        for (const auto& chId : chList) chMembers[chId] = {};

        for (auto& ns : nodeStates) {
            if (ns.isClusterHead) continue;
            uint32_t bestCH  = chList[0];
            double   bestDist = 1e18;
            for (uint32_t cid : chList) {
                for (const auto& cn : nodeStates) {
            ...
            ns.clusterId = bestCH;
            chMembers[bestCH].push_back(ns.id);
        }

        for (const auto& [chId, mems] : chMembers) {
            ClusterInfo ci;
            ci.chId    = chId;
            ci.members = mems;
            leachClusters.push_back(ci);
        }
        g_ifo.SetClustersFromExternal(leachClusters);
    }

    if (g_rl.IsConnected())
        g_rl.SendTopology(g_ifo.GetClusters());

    {
        const auto stats = g_ifo.GetStats();
        NS_LOG_UNCOND("[LEACH-noIFO] Initial : " << stats.nClusters << " clusters"
    ...
    g_compMetrics.avgPDR_RL_preFND = pdrRL_preFND;
    ...
    NS_LOG_UNCOND(  "║ Rounds IFO      : " << g_ifo.GetRound());

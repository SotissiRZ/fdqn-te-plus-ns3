...
struct SimParams {
    ...
    std::string resultsDir = "results/DQN_noFed";
};
...
public:
    ...
    void SubmitReward(const std::vector<double>& state, int action, double reward,
        ...
        if (SendLine(os.str())) {
            std::string resp = RecvLine();
            m_steps++;
            // confirme "federated": true dans sa réponse.
            // DQN-noFed → rl_server_nofed.py retourne toujours "federated": false
            // → m_fedRound reste 0 pour toute la simulation (ablation correcte).

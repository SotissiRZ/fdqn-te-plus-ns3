 ...
 *
 * Sorties dans results_eval/FDQN_TEplus/ :
...
struct SimParams {
    ...
    // failureRate : fraction de nœuds à mettre en panne (0.0 = désactivé)
    // failureTime : instant de déclenchement des pannes (en secondes simulées)
    // Scientifiquement : modélise une défaillance simultanée de type "crash fault"
    // ou "battery depletion burst" — cohérent avec les scénarios WSNBT et IoT-FT.
    double   failureRate = 0.0;    // ex: 0.1, 0.2, 0.3
    double   failureTime = 1200.0; // instant de panne (s)
};
...
// ── Robustness Failure Mechanism — variables globales ─────────────────────────
// g_failureInjected : verrou one-shot, garantit une seule injection par simulation
// g_forcedFailureNodes : ensemble des nœuds abattus par le mécanisme de test
// (distinct de g_deadNodes pour pouvoir les distinguer dans les métriques)
static bool             g_failureInjected    = false;
static std::set<uint32_t> g_forcedFailureNodes;
...
//
// Remplace ComputeAverageDelay(FlowMonitor) qui mesurait uniquement le délai
// OLSR 1-saut (~4.76 ms constant) et ne reflétait pas la topologie RL.
//
...
struct SimContext {
    ...
    // ── Robustness failure parameters (propagés depuis SimParams) ─────────────
    double      failureRate = 0.0;
    double      failureTime = 1200.0;
};
...
static void InitRLStep() {
    rlStep = [&]() {
        ...
        // Déclenchement ONE-SHOT à t >= failureTime si failureRate > 0.
        // Mécanisme : énergie forcée à 0 J sur les nœuds sélectionnés.
        // Cohérence WSN : identique à une mort naturelle par épuisement énergétique.
        // Le RL perçoit les nœuds comme morts (isAlive=false) dès le step suivant,
        // ce qui teste sa capacité de reroutage — scientifiquement défendable.
        // Protection : verrou g_failureInjected empêche toute ré-injection.
        if (!g_failureInjected && ctx.failureRate > 0.0 && now >= ctx.failureTime) {
            g_failureInjected = true;

            // Collecter les nœuds vivants candidats (exclure les déjà morts)
            std::vector<uint32_t> aliveIdx;
            aliveIdx.reserve(ctx.nNodes);
            ...
            // Nombre de nœuds à abattre
            const uint32_t nToKill = static_cast<uint32_t>(
                std::round(ctx.failureRate * aliveIdx.size()));

            // Mélange Fisher-Yates avec le RNG partagé de la simulation
            // → résultats reproductibles par seed, pas de biais de sélection
            std::shuffle(aliveIdx.begin(), aliveIdx.end(), *ctx.pRng);

            NS_LOG_UNCOND("\n╔══════════════════════════════════════════════╗");
            NS_LOG_UNCOND("║  [FAILURE INJECTION] t=" << now << "s");
            NS_LOG_UNCOND("║  failureRate=" << ctx.failureRate
                          << "  nToKill=" << nToKill
                          << "  aliveBefore=" << aliveIdx.size());
            NS_LOG_UNCOND("╚══════════════════════════════════════════════╝");

            for (uint32_t k = 0; k < nToKill && k < aliveIdx.size(); k++) {
                NodeState& ns = (*ctx.pSt)[aliveIdx[k]];

                // 1. Mettre l'énergie logique (NodeState) à 0
                // Note : on ne touche PAS à BasicEnergySource NS-3.
                // SetAttribute("InitialEnergyJ", 0) → NS_ASSERT (consumed > initial).
                // HandleEnergyDepletion() → non public dans NS-3.39.
                // La mort effective est garantie par ipv4->SetDown() ci-dessous.
                ns.energy   = 0.0;
                ns.isAlive  = false;

                // 3. Désactiver l'interface IP (identique à la mort naturelle)
                ...
                // 4. Stopper l'application UDP
                auto itApp = ctx.nodeApps.find(ns.id);
                if (itApp != ctx.nodeApps.end() && itApp->second)
                    itApp->second->SetStopTime(Seconds(now));

                // 5. Enregistrer dans les deux ensembles de nœuds morts
                g_deadNodes.insert(ns.id);
                g_forcedFailureNodes.insert(ns.id);

                // 6. Mise à jour des métriques réseau lifetime (FND/HND/LND)
                if (!g_fndDone) {
                    g_fndDone = true; g_fndTime = now;
                    g_compMetrics.fndTime = now;
                    NS_LOG_UNCOND("⭐ [FND via FAILURE] Premier nœud mort (injecté) à t=" << now << "s");
                ...
                if (!g_hndDone && g_deadNodes.size() >= g_nNodes / 2) {
                    ...
                    NS_LOG_UNCOND("⭐ [HND via FAILURE] Moitié nœuds morts à t=" << now << "s");
                }
                if (!g_lndDone && g_deadNodes.size() >= (uint32_t)(g_nNodes * 0.9)) {
                    g_lndDone = true; g_lndTime = now;
                    g_compMetrics.lndTime = now;
                    NS_LOG_UNCOND("⭐ [LND-90% via FAILURE] 90% nœuds morts à t=" << now << "s");
                    Simulator::Stop();
                }

                NS_LOG_UNCOND("  ✗ Nœud " << ns.id << " tué (panne injectée)");
            }
        }
        // ── FIN FAILURE INJECTION ────────────────────────────────────────────────
...
static void InitDoCheck() {
    doCheck = [&](int round) {
        ...
        // Remplace ComputeAverageDelay(FlowMonitor) qui mesurait un délai OLSR
        // 1-saut constant (~4.76ms) insensible à la topologie RL.
...
int main(int argc, char* argv[]) {
    ...
    cmd.AddValue("failureRate", "Fraction de nœuds à mettre en panne [0.0-1.0]", p.failureRate);
    cmd.AddValue("failureTime", "Instant de déclenchement des pannes (s)",       p.failureTime);
    cmd.Parse(argc, argv);
    ...
    if (p.failureRate > 0.0) {
        NS_LOG_UNCOND("  [ROBUSTNESS] failureRate=" << p.failureRate
                      << "  failureTime=" << p.failureTime << "s"
                      << "  (~" << static_cast<uint32_t>(std::round(p.failureRate * p.nNodes))
                      << " nœuds seront abattus à t=" << p.failureTime << "s)");
    }
    NS_LOG_UNCOND("────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────");
    ...
    // ── Robustness parameters propagation ────────────────────────────────────
    ctx.failureRate  = p.failureRate;
    ctx.failureTime  = p.failureTime;
    ...
    // Export résumé final
    {
        ...
        summary << "# FDQN-TE+ Summary\n"
                ...
                << "ForcedFailureNodes," << g_forcedFailureNodes.size() << "\n"
                << "FailureRate," << p.failureRate << "\n"
                << "FailureTime_s," << p.failureTime << "\n"

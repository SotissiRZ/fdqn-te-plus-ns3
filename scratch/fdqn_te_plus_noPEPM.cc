...
struct SimParams {
    ...
    std::string resultsDir = "results/DQN_noPEPM";
};
...
static void InitRLStep() {
    rlStep = [&]() {
        ...
        // pepmRisk = 0.0 fixe pour tous les nœuds. Aucune requête TCP pepm_batch.
        // La reconnexion reste active pour l'agent ADDQN (action/reward).
        // ─────────────────────────────────────────────────────────────────────────
        if (!g_rl.IsConnected())
            g_rl.TryReconnect();

        for (uint32_t i = 0; i < ctx.nNodes; i++)
            (*ctx.pSt)[i].pepmRisk = 0.0;
        // ─────────────────────────────────────────────────────────────────────────
        ...
        for (uint32_t i = 0; i < ctx.nNodes; i++) {
            ...
            // Récompense
            {
                ...
                const double reward
                    ...
                    + 0.0                    // ABLATION noPEPM : LAMBDA_SAFE neutralisé (pepmRisk=0.0 fixe)
...
static void InitDoCheck() {
    doCheck = [&](int round) {
        ...
        // ABLATION noPEPM : Re-clustering proactif PEPM supprimé.
        // Sans PEPM, aucune détection de nœuds à risque → pas de rotation CH proactive.
        // Le re-clustering reste uniquement périodique (IFO round%2).

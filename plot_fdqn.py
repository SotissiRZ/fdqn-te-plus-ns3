...
def main():
    ...
    # Annotation explication divergence
    ax.annotate("Divergence : NS-3 canal idéal\nvs. routage RL sous-optimal",
                xy=(x[len(x)//2], 99.5), fontsize=8, color="gray",
                ha="center")

    vline_lifetime(ax, fnd_s, hnd_s, lnd_s)
    ax.set_xlabel("Temps (s)")
    ax.set_ylabel("PDR (%)")
    ax.set_title("FDQN-TE+ — Taux de livraison paquets\n"
                 "PDR_RL = métrique ADDQN logique | PDR_NS3 = FlowMonitor MAC/PHY")
    ax.legend()
    ...
    if len(reward_mean):
        ...
        ax.set_title("FDQN-TE+ — Courbe d'apprentissage\n"
                     "R = λ₁·PDR + λ₂·E_norm − λ₃·delay − λ₄·risk + λ₅·hier")
        ax.legend()
        ax.grid(True, alpha=0.3)
    ...
    if len(e_total):
        ...
        ax.set_title("FDQN-TE+ — Consommation énergétique cumulée\n"
                     "Modèle LEACH (E_elec=50 nJ/bit, E_amp=10 pJ/bit/m²)")
        ax.legend()
        ax.grid(True, alpha=0.3)
        ax.set_xlim(left=0)
    ...
    if len(pepm_risk) and pepm_risk.max() > 0:
        ...
        ax.axhline(0.5, color=C_PEPM, linestyle="--", linewidth=1, alpha=0.6,
                   label="Seuil alerte (0.5)")
        ax.fill_between(x, 0.5, pepm_risk,
                        where=(pepm_risk > 0.5), color=C_PEPM, alpha=0.15,
                        label="Zone à risque")
    ...
    ax.set_title("FDQN-TE+ — Mécanisme PEPM (Predictive Energy Proactive)\n"
                 "Risque = f(LSTM + EWMA + absolu) — seuil = 0.5 → rotation CH anticipée")
    ax.set_ylim(0, 1.05)
    ax.legend(loc="upper right")
    ax.grid(True, alpha=0.3)
    ...
    ax.set_title("FDQN-TE+ — Clusters IFO actifs\n"
                 "Diminue naturellement à mesure que les nœuds meurent (ComputeNClusters adaptatif)")
    ax.legend()
    ...
    if len(fed_round) and fed_round.max() > 0:
        ...
        ax.set_title("FDQN-TE+ — Fédération FedMeta-DRL\n"
                     "1 round fédéré tous les 50 steps RL | Agrégation 2 niveaux (intra/global)")
        ax.legend()
        ax.grid(True, alpha=0.3)
